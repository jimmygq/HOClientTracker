...
const queries = {
  ...
  async getStats() {
    ...
    const [totalOpen, blocked, highPriority, overdue, resolvedAllTime, resolvedThisWeek, avgResolve] = await Promise.all([
      ...
      db.execute("SELECT COUNT(*) as c FROM requests WHERE status = 'Resolved'"),
      db.execute({ sql: "SELECT COUNT(*) as c FROM requests WHERE status = 'Resolved' AND COALESCE(resolved_at, last_updated) >= ?", args: [oneWeekAgo] }),
      db.execute("SELECT AVG(julianday(COALESCE(resolved_at, last_updated)) - julianday(date_started)) as avg_days FROM requests WHERE status = 'Resolved'"),
    ]);
    const avgDays = avgResolve.rows[0].avg_days;
    return {
      ...
      resolvedAllTime: Number(resolvedAllTime.rows[0].c),
      resolvedThisWeek: Number(resolvedThisWeek.rows[0].c),
      avgResolveDays: avgDays != null ? Math.round(Number(avgDays) * 10) / 10 : null,
    };
  },
