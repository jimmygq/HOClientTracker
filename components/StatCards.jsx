...
function Card({ label, value, accent, icon }) {
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm flex items-start gap-4">
      <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${accent.bg}`}>
      ...
      <div>
        ...
        <div className={`text-2xl font-bold mt-0.5 ${accent.text}`}>{value ?? '—'}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
      ...
      <Card label="Resolved / Week" value={stats?.resolvedThisWeek}
        accent={{ bg: 'bg-teal-50', icon: 'text-teal-500', text: 'text-gray-900' }}
